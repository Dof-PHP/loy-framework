...
在阅读本文档的时候，所有关于请求参数类型标识（包括请求参数和响应参数）的含义，均在下面的列表中说明：
...
| `IN` | 指定的参数必须在给定的枚举列表中 | - | `status` => 'in:1,2,3' 表示名为 `status` 的参数必须是 `1`/`2`/`3` 的某一个|
...
| `WRAPIN` | 该参数值引用了另外一个已定义的数据格式集合 | 隐式会判断基本数据类型是否是数组 | `Domain\User\Http\Wrapper\In\ABC` |
