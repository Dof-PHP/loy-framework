...
/**
 * DOF-PHP Builtin Exceptions
 *
 * #0: int; Global unique exception code
 * #1: string; Global exception name
 * #2: string; Exception default description in a language
 * #3: string; Exception default suggestion in a language
 */
class EXCP
{
    const DOF_SERVICE_EXCEPTION = [50300000, Service::EXCEPTION];
    ...
    const VIOLATED_UNIQUE_CONSTRAINT = [50000030, 'ViolatedUniqueConstraint'];
    ...
    const HTTP_HMAC_TOKEN_VERIFY_FAILED = [40100007, 'HttpHmacTokenVerifyFailed'];
    ...
    const TIMEOUTED_HMAC_SIGNATURE = [40100011, 'TimeoutedHMACSignature'];

    const INVALID_REQUEST_MIME = [40000000, 'InvalidRequestMime'];
    const WRAPIN_VALIDATE_FAILED = [40000001, 'WrapinValidateFailed'];
    const INVALID_ROUTE_PARAMETER = [40000002, 'InvalidRouteParameter'];
    const INVALID_AUTH_CLIENT_ID = [40000003, 'InvalidAuthClientId'];
    const INPUT_FIELDS_SENTENCE_GRAMMER_ERROR = [40000004, 'InputFieldsSentenceGrammerError'];
    const INVALID_AUTH_CLIENT_REALM = [40000005, 'InvalidAuthClientRealm'];
    const INVALID_SORTING_FIELD = [40000006, 'InvalidSortingField'];

    const NOTHING_TO_UPDATE = [20200002, 'NothingToUpdate'];
}
