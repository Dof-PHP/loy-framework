...
class IS
{
    public static function bint($value, bool $force = true) : bool
    {
        ...
        return TypeHint::isBint($value);
    }

    public static function uint($value, bool $force = true) : bool
    {
        if ($force) {
            return is_int($value) && ($value >= 0);
        }

        return TypeHint::isUint($value);
    }

    public static function pint($value, bool $force = true) : bool
    {
        if ($force) {
            return is_int($value) && ($value > 0);
        }

        return TypeHint::isPint($value);
    }
