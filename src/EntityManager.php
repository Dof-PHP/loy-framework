...
final class EntityManager
{
    ...
    public static function compile(array $dirs, bool $cache = false)
    {
        ...
        if ($cache) {
            array2code([self::$dirs, self::$entities], Kernel::formatCompileFile(__CLASS__));
        }
    }
    ...
    public static function __annotationMultipleMergeOnCreated()
    {
        return 'index';
    }

    public static function __annotationMultipleMergeOnRemoved()
    {
        return 'index';
    }

    public static function __annotationMultipleMergeOnUpdated()
    {
        return 'index';
    }

    public static function __annotationMultipleOnCreated() : bool
    {
        return true;
    }

    public static function __annotationMultipleOnRemoved() : bool
    {
        return true;
    }

    public static function __annotationMultipleOnUpdated() : bool
    {
        return true;
    }

    public static function __annotationFilterOnCreated(string $listener, array $ext = [], string $entity = null) : array
    {
        $_listener = get_annotation_ns($listener, $entity);
        if (! $_listener) {
            exception('EntityOnCreatedEventListenerNotExists', compact('listener', 'entity'));
        }
        if (! is_subclass_of($_listener, Listener::class)) {
            exception('InvalidEntityOnCreatedEventListener', compact('listener', 'entity'));
        }

        return [$_listener => $ext];
    }

    public static function __annotationFilterOnRemoved(string $listener, array $ext = [], string $entity = null) : array
    {
        $_listener = get_annotation_ns($listener, $entity);
        if (! $_listener) {
            exception('EntityOnRemovedEventListenerNotExists', compact('listener', 'entity'));
        }
        if (! is_subclass_of($_listener, Listener::class)) {
            exception('InvalidEntityOnRemovedEventListener', compact('listener', 'entity'));
        }

        return [$_listener => $ext];
    }

    public static function __annotationFilterOnUpdated(string $listener, array $ext = [], string $entity = null) : array
    {
        $_listener = get_annotation_ns($listener, $entity);
        if (! $_listener) {
            exception('EntityOnUpdatedEventListenerNotExists', compact('listener', 'entity'));
        }
        if (! is_subclass_of($_listener, Listener::class)) {
            exception('InvalidEntityOnUpdatedEventListener', compact('listener', 'entity'));
        }
