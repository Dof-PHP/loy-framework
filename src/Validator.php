...
class Validator
{
    ...
    public function execute()
    {
        foreach ($this->rules as $key => $rules) {
            ...
            if (! $need) {
                // If no need and parameter key exists but equals to empty string
                // Just unset this key in result to ignore this parameter
                if ($val === '') {
                    // unset($this->data[$key]);
    ...
    private function validateFormatwhen(string $key, string $format)
    {
        ...
        if (! method_exists(IS::class, $format)) {
            exception('ISFormatterNotExists', compact('format'));
        }

        return IS::$format($this->data[$key] ?? null);
    }

    private function validateRegexwhen(string $key, string $regex)
    {
