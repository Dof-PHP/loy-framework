...
class Command
{
    ...
    {
        $console->line();
        $console->output($console->render('(c) DOF PHP', 'LIGHT_BLUE'));
        $console->output('  ');
    ...
    public function testStorageConnection($console)
    {
        ...
        foreach ($storages as $storage) {
            ...
            if ($output) {
                ...
                $file = DomainManager::getRelative($file);
                $console->info($file);
            }
        }
    ...
    {
        $fails = [];
        walk_dir_recursive(ospath(Kernel::getRoot(), DomainManager::DOMAIN_PATH), function ($path) use ($console, &$fails) {
            if (! ci_equal($path->getExtension(), 'php')) {
                return;
            }
            $file = $path->getRealpath();
            $class = get_namespace_of_file($file, true);
            if (false === $class) {
                return;
            }
            ...
            try {
                ...
                $console->output($console->render('Failed: ', $console::ERROR_COLOR));
                $context = parse_throwable($e);
                $context['namespace'] = $class;
                $context['filepath'] = $file;
                $fails['Exception'][] = $context;
            }

            if ($output) {
                $console->info(DomainManager::getRelative($file));
            }
        });
    ...
    */
    ...
    {
        walk_dir_recursive(ospath(Kernel::getRoot(), DomainManager::DOMAIN_PATH), function ($path) {
            if (! ci_equal($path->getExtension(), 'php')) {
                return;
            }
            $file = $path->getRealpath();
            ...
            // TODO
            // pd(token_name(320));
            // pd($file, token_get_all(php_strip_whitespace($file)));
        });
    }
    ...
    public function listPort($console)
    {
        ...
        $domains = array_trim_from_string($domains, ',');
        if ($domains) {
            foreach ($domains as &$domain) {
        ...
        $verbs = array_trim_from_string($console->getOption('verb', ''));
        array_walk($verbs, function (&$verb) {
            $verb = strtoupper($verb);
        });
        $versions = array_trim_from_string($console->getOption('version', ''));
        $logging = $console->getOption('logging');

        $result = [];
        $routes = PortManager::getRoutes();
        $ports  = PortManager::getPorts();
        foreach ($routes as $group) {
            foreach ($group as $verb => list('class' => $class, 'method' => $method, 'urlpath' => $urlpath)) {
                $_domain = DomainManager::getKeyByNamespace($class);
                if (! $_domain) {
                    $console->exception('BadPortClassWithOutDomain', compact('urlpath', 'verb', 'class'));
                }
                $title = $ports[$class][$method]['title'] ?? null;
                if (! $title) {
                    $console->exception('BadPortWithOutTitle', compact('class', 'method', 'urlpath', 'verb'));
                }
                $_logging = $ports[$class][$method]['logging'] ?? null;
                if (! is_null($logging)) {
                    if (confirm($logging) && (! $_logging)) {
                        continue;
                    }
                    if ((! $logging) && $_logging) {
                        continue;
                ...
                if ($versions && (! ciin($version, $versions))) {
                    continue;
                }
                ...
                if ($_port = ($result[$port] ?? null)) {
                    $_verbs = array_unique_merge(explode('|', $_port[0] ?? ''), [$verb]);
                    $_verbs = join('|', $_verbs);
                }

                $result[$port] = [$_verbs, $urlpath, $title];
            }
        }
        ...
        if ($console->hasOption('count')) {
            $console->info((string) $count, true);
        }
        ...
        switch (strtolower($format)) {
            case 'array':
                $save
                    ...
                    : $console->success(get_buffer_string(function () use ($data) {
                        print_r($data);
                    }), true);
                break;
            case 'xml':
                $save
                    ...
                    : $console->success(xml_pretty($data), true);
                break;
            case 'json':
            default:
                $save
                    ? save($save, enjson($data))
                    : $console->success(json_pretty($data), true);
                break;
        }

        $save && $console->success('Done!');
    }
    ...
    */
    ...
    public function addEntity($console)
    {
        ...
        $name = $console->getOption('entity', null, true);
        ...
        $entity = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $entity);
        ...
        $__class = DomainManager::getRelative($class);

        $console->line(
            $console->render('Created Entity: ', $console::SUCCESS_COLOR)
            .$console->render("{$_class} ({$__class})", $console::INFO_COLOR)
        );
    }
    ...
    public function addDM($console)
    {
        ...
        $name = $console->getOption('model', null, true);
        $name = underline2camelcase($name);
        ...
        $model = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $model);
    ...
    public function addPort($console)
    {
        ...
        $name = $console->getOption('port', null, true);
        $name = underline2camelcase($name);
        ...
        $port = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $port);
    ...
    public function addWrapIn($console)
    {
        ...
        $pipe = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $pipe);
    ...
    public function addORMStorage($console)
    {
        ...
        $orm = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $orm);
        $orm = str_replace('__NAMESPACE__', path2ns($name), $orm);
        $orm = str_replace('__DATABASE__', camelcase2underline($domain), $orm);
        $orm = str_replace('__TABLE__', camelcase2underline(basename($name)), $orm);
    ...
    public function addKVStorage($console)
    {
        ...
        $kv = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $kv);
    ...
    public function addRepository($console)
    {
        ...
        $repo = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $repo);
    ...
    public function addService($console)
    {
        ...
        $name = $console->getOption('service', null, true);
        $name = underline2camelcase($name);
        ...
        $service = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $service);
    ...
    public function addAssembler($console)
    {
        ...
        $name = $console->getOption('asm', null, true);
        $name = underline2camelcase($name);
        ...
        $assembler = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $assembler);
    ...
    /**
     * @CMD(ers)
     * @Desc(Create Entity/Repository/ORM-Storage at once)
     * @Option(domain){notes=Domain name of classes to be created}
     * @Option(entity){notes=Entity directory path}
     * @Option(storage){notes=ORM storage directory path}
     * @Option(repo){notes=Repository directory path}
    ...
    public function mrs($console)
    {
        ...
        $_model = basename($model);

        $storage = $console->hasOption('storage')
            ? join('/', [$console->getOption('storage'), $_model])
            : $_model;

        $repo = $console->hasOption('repo')
            ? join('/', [$console->getOption('repo'), $_model])
            : $_model;
        $console->setOption('storage', $storage)
                ->setOption('type', 'kv')
                ->setOption('repo', $repo);
        $this->addRepository($console);
...
PHP;
    ...
    public function addCMD($console)
    {
        ...
        $command = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $command);
    ...
    public function addEvent($console)
    {
        ...
        $name = $console->getOption('event', null, true);
        $name = underline2camelcase($name);
        ...
        $event = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $event);
    ...
    public function addListener($console)
    {
        ...
        $name = $console->getOption('listener', null, true);
        $name = underline2camelcase($name);
        ...
        $listener = str_replace('__DOMAIN__', DomainManager::formatKey($domain), $listener);
    ...
    public function getNamespaceOfFile($console)
    {
        ...
        if (namespace_exists($target)) {
            $file = get_file_of_namespace($target);
            $file = ($file && (! $console->hasOption('full')))
                ? DomainManager::getRelative($file)
                : $file;
            $file ? $console->success($file) : $console->warning('nil');
            return;
        }
