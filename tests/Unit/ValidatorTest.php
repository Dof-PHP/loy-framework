...
GWT('测试validator的check方法', [
    ...
    return \Loy\Framework\Facade\Validator::setData(['id' => 01])->setRules($given)->execute();
}, function ($result) {
...
GWT('测试2', [
    ...
    return \Loy\Framework\Facade\Validator::setData(['id' => 1])->setRules($given)->execute();
}, function ($result) {
    return ($result->getFails()->first()->key ?? false) === 'id不在配置中: 1';
});
...
GWT('测试4', [
    ...
    return \Loy\Framework\Facade\Validator::setData(['id' => 22])->setRules($given)->execute();
}, function ($result) {
